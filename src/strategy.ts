...
export function decideMarket(input: StrategyInput): MarketDecision {
  ...
  const oneBarMoveBps = previousCandle ? Math.abs(latestCandle.close / previousCandle.close - 1) * 10_000 : 0;
  ...
  const buyAlpha = signalBps >= feeAdjustedThreshold;
  const sellAlpha = signalBps <= -feeAdjustedThreshold;
  ...
  const buyNotional = Math.min(baseNotional, cashBudget, grossBudget, marketBudget);
  const desiredOrders: DesiredOrder[] = [];
  const stopLoss =
    positionQuantity > 0 && Number(position?.propertyPnlPercent ?? 0) <= -Math.abs(config.stopLossPct);

  if (!stopLoss && (marketMakingEdge || buyAlpha) && buyNotional >= config.minOrderNotional) {
  ...
  if (positionQuantity > 0 && (stopLoss || marketMakingEdge || sellAlpha)) {
    const sellNotional = stopLoss ? positionQuantity * bestBid.price : Math.min(baseNotional, positionQuantity * bestAsk.price);
    const improvedAsk = Math.max(bestBid.price + config.tickSize, bestAsk.price - config.tickSize);
    const passiveTarget = reservationPrice * (1 + Math.max(config.minNetEdgeBps, volatilityBps * 0.12) / 10_000);
    const rawAsk = stopLoss
      ? bestBid.price
      : sellAlpha
        ? improvedAsk
        : Math.max(improvedAsk, Math.min(bestAsk.price, passiveTarget));
    const price = roundToTick(Math.max(rawAsk, stopLoss ? bestBid.price : bestBid.price + config.tickSize), config.tickSize, "up");
    const requestedQuantity = stopLoss
      ? positionQuantity
    ...
    if (quantity > 0 && quantity * price >= config.minOrderNotional && (stopLoss || price > bestBid.price)) {
      desiredOrders.push({
        ...
        rationale: [stopLoss ? "stop-loss" : marketMakingEdge ? "fee-adjusted-spread" : "negative-alpha"],
      });
    }
  }
